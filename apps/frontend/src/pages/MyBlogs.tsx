      ...
      <main className="relative max-w-7xl mx-auto px-6 pt-16 md:pt-24">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div className="space-y-4">
              ...
              <h1 className="text-6xl md:text-8xl font-black tracking-tighter uppercase leading-[0.85]">
                My<br />Stories<span className="text-black/10">.</span>
              </h1>
            </div>
            ...
            <div className="backdrop-blur-xl bg-white/40 border border-black/[0.08] p-6 rounded-[2rem] shadow-xl flex items-center gap-8 min-w-[300px]">
              <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-black/30">Total</p>
                <p className="text-2xl font-black leading-none">{blogs.length}</p>
              </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((n) => (
              <div key={n} className="h-80 rounded-[2.5rem] bg-black/[0.03] animate-pulse border border-black/[0.05]" />
            ))}
          </div>
        ) : blogs.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <span className="text-xs font-black uppercase tracking-[0.2em] text-black/40 group-hover:text-black">New Story</span>
            </Link>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-40 border border-dashed border-black/10 rounded-[3rem] bg-black/[0.01]">
            ...
            <h2 className="text-xl font-black uppercase tracking-widest text-black/20 mb-8">No articles created yet</h2>
            <Link to="/write" className="bg-black text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-transform">
              Start Writing
            </Link>
          </div>
        )}
