...
const BlogDetail = () => {
  ...
  if (isLoading) return (
    <div className="flex flex-col justify-center items-center h-screen space-y-4">
      ...
      <p className="text-xs font-black uppercase tracking-widest text-black/40">Loading Story...</p>
    </div>
  );
  ...
  if (error || !blog) return (
    <div className="flex flex-col justify-center items-center h-screen text-center px-4">
      ...
      <button onClick={() => navigate(-1)} className="text-sm font-bold border-b-2 border-black pb-1">Go Back</button>
    </div>
  );
    ...
    <article className="min-h-screen bg-white text-black pb-20">
      <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className={`flex items-center gap-1 text-[10px] font-black uppercase px-3 py-1 rounded-full border ${
              blog.published ? "bg-black text-white" : "bg-black/5 text-black/40"
            }`}>
      ...
      <main className="max-w-3xl mx-auto px-6 pt-16 md:pt-24">
        <header className="mb-12 space-y-6">
          ...
          <div className="flex items-center gap-6 text-black/40 pt-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider">{blog.author?.name || "Author"}</span>
            </div>
          </div>
        </header>
        ...
        <footer className="mt-20 pt-10 border-t border-black/5">
          <div className="bg-black/[0.02] p-8 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-black/30 mb-1">Finished Reading?</p>
              <h4 className="text-xl font-bold">Share this story or explore more.</h4>
            </div>
            <button onClick={()=>{
              navigate("/")
